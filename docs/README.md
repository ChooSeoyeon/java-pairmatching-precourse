...
- [ ] `원하는 기능을 입력` 받는다 (`String` → `Function(enum)`)
    - [ ] `존재하는 기능이 아닌 경우`, 예외를 발생시킨다
- [ ] `과정, 레벨, 미션을` 쉼표로 구분해 `입력` 받는다 (`String` → `Target(dto)`)
    - [ ] `존재하지 않는 과정, 레벨, 미션인 경우`, 예외를 발생시킨다
