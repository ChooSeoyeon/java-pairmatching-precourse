...
- [x] 과정/레벨/미션으로 이전 매칭 결과를 조회한다
- [x] 과정/레벨으로 이전 매칭 결과 존재 유무를 조회한다
- [x] 과정/레벨으로 이전 매칭 결과를 조회한다
...
- [x] `재매칭 여부를 입력` 받는다 (`String` → `boolean`)
    - [x] `네` 또는 `아니오`가 아닌 경우, 예외를 발생시킨다

### OutputView

- [x] `기능 선택 문구`를 출력한다
- [x] `과정, 레벨, 미션 선택 문구`를 출력한다
- [x] `재매칭 선택 문구`를 출력한다
- [x] `예외 메시지`를 출력한다
- [x] `패어 매칭 결과`를 출력한다
- [x] `페어 조회 결과`를 출력한다
